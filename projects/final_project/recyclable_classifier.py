...
# Function to plot an image with a title
...
image_batch, label_batch = test_generator[index_to_plot]  # Get batch at the specified index
image = image_batch[0]  # First image in the batch
true_label = label_batch[0]  # True label for the first image in the batch

# Predicted label
predicted_label = predictions_extract_feat_model[index_to_plot]

# Display the image with title
title = f"{model_name}: True Label = {true_label}, Predicted Label = {predicted_label}"
...
# Get the image and true label from the test generator
image_batch, label_batch = test_generator[index_to_plot]  # Get batch at the specified index
image = image_batch[0]  # First image in the batch
true_label = label_batch[0]  # True label for the first image in the batch
